...
/**
 * Retries an async function with exponential backoff. Handles transient network/page load failures.
 * @param fn - Function to retry
 * @param description - Description for logging context
...
async function processCompany(companyName: string): Promise<CompanyData> {
  ...
  try {
    ...
    // Start from Google to allow CUA agent to search and navigate to company homepage
    ...
    // Use CUA (Computer Use Agent) for autonomous navigation - handles search, clicks, and navigation
    ...
    // Extract both links in parallel since they're independent operations
    ...
    // Try Terms of Service first (more likely to contain physical address for legal contact info)
